...
measure_index = find(strcmp(T.MEASURE_NAME, 'Count_Recovery'));

% extract the specified metric
specified_metric = table2array(T(measure_index, 2:end));

other_metric = table2array(T(:, 2:end));
[corr_mat, pval] = corr(specified_metric', other_metric', 'Type', 'Spearman');


% outputting the metrics with above 0.7 or below -0.7
results = [T(:,1), array2table(corr_mat')];
